...
├── index.mdx          ← 本文（必須）
├── images/            ← 画像置き場
│   ├── cover.jpg     ← カバー画像（必須・1枚）
│   ├── 01.jpg        ← ギャラリー画像（連番でソート）
│   ├── 02.jpg
│   └── 03.png
...
- **`cover.{jpg,jpeg,png,webp,avif,gif}`** — 必須。一覧カードとプロジェクト詳細ヒーローに使われる。
- **`数字始まり.{拡張子}`** — ギャラリー画像。`01.jpg` `02-detail.jpg` のようにファイル名先頭が数字で始まるものが自動で連番ソートされてギャラリーに並ぶ。
